...
vec3 fxaa(vec2 p) {
  ...
  vec3 rgbA = .5 * (texture2D(samplerFront, p + dir * (1. / 3. - .5)).rgb + texture2D(samplerFront, p + dir * (2. / 3. - .5)).rgb);
  vec3 rgbB = rgbA * .5 + .25 * (texture2D(samplerFront, p + dir * (0. / 3. - .5)).rgb + texture2D(samplerFront, p + dir * (3. / 3. - .5)).rgb);
